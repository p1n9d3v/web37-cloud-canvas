...
const VPC3D = ({ bounds, name, color }: Props) => {
...
const VPC2D = ({ bounds, color, name }: Props) => {
...
export default ({ bounds, name }: Pick<Props, 'bounds' | 'name'>) => {
    ...
    return dimension === '2d' ? (
        <VPC2D bounds={bounds} name={name} color={color} />
    ) : (
        <VPC3D bounds={bounds} name={name} color={color} />
    );
};
